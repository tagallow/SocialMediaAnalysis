...
var api = {
    socialMonitorCloudURL: protocol + '//socialmonitor.whateverittakes.com/api/socialmonitor?',
    ...
    drawChart_noflash: function (keyword, volumeDiv, moodDiv, historyDiv) {
        ...
        $.getJSON(
            ...
            function (data) {
                ...
                moodData.addColumn({ type: 'string', role: 'annotationText', p: { html: true } });
                ...
                for (var i = 0; i < data['dates'].length; i++) {
                    ...
                    moodData.addRow([new Date(data['dates'][i]), mood, data['annotationTitles'][i], data['annotations'][i]]);
                ...
                var volumeoptions = {
                    ...
                    hAxis: { textPosition: 'out' },
                    vAxis: { textPosition: 'left', gridlines: { count: 3 }, title: "Volume" },
                    chartArea: { left: 100, top: 10, width: "90%", height: "70%" },
                };

                var moodChart = new google.visualization.LineChart(document.getElementById(moodDiv));
                var volumeChart = new google.visualization.AreaChart(document.getElementById(volumeDiv));
                ...
                function selectHandler(isMood) {
                    ...
                    if(isMood)
                        selectedItem = moodChart.getSelection()[0];
                    else
                        selectedItem = volumeChart.getSelection()[0];
                    ...
                    if (selectedItem) {
                        ...
                        api.currentRequest = $.get(url, '', function (data) {
                            ...
                            for (var i = 0; i < data["sentimentList"].length; i++) {
                                ...
                                var datePosted = new Date(data["sentimentList"][i]["time"]).toLocaleString();
                                var authorURL = protocol+'//twitter.com/' + author;
                                var tweetURL = authorURL + '/status/';
                                if (i != 0) {
                                    $(historyDiv).append('<br><br /><hr style="height: 2px;background-color: #ccc">');
                                }
                                if (data["sentimentList"][i]['source'] != 'Unknown')

                                    $(historyDiv).append('<img src="' + protocol + '//socialmonitor.whateverittakes.com/images/social_media_icons/' + data["sentimentList"][i]['source'] + '.png" width="75" height="75" align="left">');
                                //$(historyDiv).append('<img src="/images/social_media_icons/' + data["sentimentList"][i]['source'] + '.png" width="75" height="75" align="left">');

                                $(historyDiv).append('<b><a href="' + authorURL + '" target="_blank">' + author + '</a></b><br /><font color="grey">' + datePosted + '</font><br />' + text + '<br /><br />');
                                //'#' + divID + 'TwitFeed'
                            }
                ...
                google.visualization.events.addListener(moodChart, 'select', selectHandlerMood);
                google.visualization.events.addListener(volumeChart, 'select', selectHandlerVol);
    ...
    getTimeRange: function (keyword, range, volumeDiv, moodDiv) {
        ...
        $.getJSON(url, '', function (data) {
            ...
            function selectHandler() {
                ...
                if (selectedItem) {
                    ...
                    $.get(url, '', function (data) {
                        ...
                        for (var i = 0; i < data["sentimentList"].length; i++) {
                            if (i == 0) {
                                api.latestDate = new Date(data['sentimentList'][i]['time']).toLocaleString();
                            }
                            var text = data["sentimentList"][i]['text'];
                            var author = data["sentimentList"][i]['author'];
                            var datePosted = new Date(data["sentimentList"][i]["time"]).toLocaleString();
                            ...
                            var tweetURL = authorURL + '/status/';
                            if (i != 0) {
                                $('#tweets').append('<br><br /><hr style="height: 2px;background-color: #ccc">');
                            }
                            if (data["sentimentList"][i]['source'] != 'Unknown')
                                
                                $('#tweets').append('<img src="'+ protocol + '//socialmonitor.whateverittakes.com/images/social_media_icons/' + data["sentimentList"][i]['source'] + '.png" width="75" height="75" align="left">');
                                //$('#tweets').append('<img src="/images/social_media_icons/' + data["sentimentList"][i]['source'] + '.png" width="75" height="75" align="left">');

                            $('#tweets').append('<b><a href="' + authorURL + '" target="_blank">' + author + '</a></b><br /><font color="grey">' + datePosted + '</font><br />' + text + '<br /><br />');
                            //'#' + divID + 'TwitFeed'
                        }
                        if (data["sentimentList"].length == 0) {
            ...
            google.visualization.events.addListener(moodChart, 'select', selectHandler);
            google.visualization.events.addListener(volumeChart, 'select', selectHandler);
            moodChart.draw(moodData, moodoptions);
            volumeChart.draw(volumeData, volumeoptions);
            if (data['sentimentList'] != null) {
                ...
                for (var i = 0; i < data['sentimentList'].length; i++) {
                    if (i == 0) {
                        api.latestDate = new Date(data["dates"][i]).toLocaleString();
                    }
                    var text = data['sentimentList'][i]['text'];
                    var author = data['sentimentList'][i]['author'];
                    ...
                    if (i != 0) {
                        $('#tweets').html('<br><br><hr style="height: 2px;background-color: #ccc"><br>' + $('#tweets').html());
                    }

                    $('#tweets').html('<b><a href="' + authorURL + '" target="_blank">' + author + '</a></b><br /><font color="grey">' + new Date(parseInt(data['sentimentList'][i]['timeString'])).toLocaleTimeString() + '</font><br />' + text + '<br /><br>' + $('#tweets').html());
                    if (data["sentimentList"][i]['source'] != 'Unknown')
                        $('#tweets').html('<img src="'+protocol+ '//socialmonitor.whateverittakes.com/images/social_media_icons/' + data["sentimentList"][i]['source'] + '.png" width="75" height="75" align="left">' + $('#tweets').html());
                    //'#' + divID + 'TwitFeed'
                }
                if (data['sentimentList'].length > 0) {
    ...
    live: function (keyword, source, tweetDiv,historyDiv) {
        ...
        function addRow() {
            ...
            $.getJSON(url, '', function (data) {
                ...
                if (data['sentimentList'] != null) {
                    for (var i = 0; i < data['sentimentList'].length && i < 10; i++) {
                        var text = data['sentimentList'][i]['text'];
                        var author = data['sentimentList'][i]['author'];
                        var datePosted = data['sentimentList'][i]['timeString'];
                        var tweetID = data['sentimentList'][i]['twitterID'];
                        ...
                        var tweetURL = authorURL + '/status/' + tweetID;
                        if (i != 0) {
                            $(tweetDiv).html('<br><hr style="height: 2px;background-color: #ccc"><br>' + $(tweetDiv).html());
                        }


                        $(tweetDiv).html('<b><a href="' + authorURL + '" target="_blank">' + author + '</a></b><br /><font color="grey">' + new Date().toLocaleTimeString() + '</font><br />' + text + '<br /><br><br><br>' + $(tweetDiv).html());
                        if (data["sentimentList"][i]['source'] != 'Unknown')
                            $(tweetDiv).html('<br><img src="'+protocol+'//socialmonitor.whateverittakes.com/images/social_media_icons/' + data["sentimentList"][i]['source'] + '.png" width="75" height="75" align="left">' + $(tweetDiv).html());
                        //'#' + divID + 'TwitFeed'
                    }
                    if (data['sentimentList'].length > 0) {
                        $(tweetDiv).show("fade", "slow");
    ...
    nextPage: function(keyword,div) {
        ...
        api.currentPage++;
        api.getHistoryPage(keyword, api.latestDate.toLocaleString(), api.currentPage, div);
    },
    ...
    getHistoryPage: function(keyword, date, page,div) {
        ...
        $.get(url, '', function (data) {
            ...
            if (data['sentimentList'].length > 0) {
                $(div).hide("fade", "fast");
                $(div).html('<hr style="height: 2px;background-color: #ccc">' + $(div).html());
            }
            if (data['sentimentList'] != null) {
                for (var i = 0; i < data['sentimentList'].length; i++) {
                    var text = data['sentimentList'][i]['text'];
                    ...
                    var tweetURL = authorURL + '/status/' + tweetID;
                    if (i != 0) {
                        $(div).html('<br><hr style="height: 2px;background-color: #ccc"><br>' + $(div).html());
                    }

                    $(div).html('<b><a href="' + authorURL + '" target="_blank">' + author + '</a></b><br /><font color="grey">' +data['sentimentList'][i]['timeString'] + '</font><br />' + text + '<br /><br/><br/>' + $(div).html());
                    if (data["sentimentList"][i]['source'] != 'Unknown')
                        $(div).html('<img src="' + protocol + '//socialmonitor.whateverittakes.com/images/social_media_icons/' + data["sentimentList"][i]['source'] + '.png" width="75" height="75" align="left">' + $(div).html());
                    //'#' + divID + 'TwitFeed'
                }
                if (data['sentimentList'].length > 0) {
                    $(div).show("fade", "fast");
